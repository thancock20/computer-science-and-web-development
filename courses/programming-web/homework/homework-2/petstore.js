...
function calculateFoodOrder(numAnimals, avgFood) {
  return isNaN(Number(numAnimals)) || isNaN(Number(avgFood)) ? -1 : numAnimals * avgFood;
}
...
function mostPopularDays(week) {
  ...
  week.forEach(function(day) {
    if (day.traffic === mostTraffic) {
      mostPopular.push(day.name);
    } else if (day.traffic > mostTraffic) {
      mostTraffic = day.traffic;
      mostPopular = [day.name];
    }
  })
  return mostPopular.length === 1 ? mostPopular[0] : mostPopular;
}
