...
function ItemDAO(database) {
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        if (category === "All") {
          col.find().sort({ _id: 1 }).skip(toSkip).limit(itemsPerPage).toArray(function(err, pageItems) {
            callback(pageItems);
          });
          return;
        }
        col.find({ category: category }).sort({ _id: 1 }).skip(toSkip).limit(itemsPerPage).toArray(function(err, pageItems) {
    ...
    this.getNumItems = function(category, callback) {
        ...
        var col = this.db.collection('item');
        if (category === 'All') {
          col.count(function(err, numItems) {
            callback(numItems);
          });
          return;
        }
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var col = this.db.collection('item');
        var toSkip = page * itemsPerPage;
        col.find({ $text: { $search: query }}).sort({ _id: 1 }).skip(toSkip).limit(itemsPerPage).toArray(function(err, items) {
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        var col = this.db.collection('item');
        col.find({ $text: { $search: query } }).count(function(err, numItems) {
          /*
          * TODO-lab2B
          *
    ...
    this.getItem = function(itemId, callback) {
         ...
         */
         ...
         var col = this.db.collection('item');
         col.findOne({ _id: itemId }, function(err, item) {
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
        ...
        var col = this.db.collection('item');
        col.update({ _id: itemId }, { $push: { reviews: reviewDoc } }, {}, function(err, doc) {
          // TODO Include the following line in the appropriate
